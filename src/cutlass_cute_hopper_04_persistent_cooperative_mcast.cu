...
struct CollectiveMainloop
{
    ...
    static constexpr int kClusterM = Kernel_traits::kClusterM;
    ...
    )
    {
        ...
        auto [tBgB, tBsB] = tma_partition(
            ...
            block_rank_in_cluster, Layout<Shape<Int<kClusterM>, _1, _1>>{},
            group_modes<0,2>(sB),
            group_modes<0,2>(gB)
        ); // (TMA, NUM_TILES_K) and (TMA, PIPE)

        constexpr uint16_t mcast_mask_B = (uint16_t(1) << kClusterM) - 1;
...
{   
    ...
    MainloopPipeline pipeline(
        ...
        pipeline_params,
        Shape<Int<Kernel_traits::kClusterM>, _1, _1>{}         // todo
    );
...
{   
    ...
    constexpr int kClusterM_ = 2;
    constexpr int kStages_ = 3;

    using Kernel_traits = KernelTraits<T, kWarps_,kBlockM_, kBlockN_, kBlockK_, kClusterM_, kStages_>;
