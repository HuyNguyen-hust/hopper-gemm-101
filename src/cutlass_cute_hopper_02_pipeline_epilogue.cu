...
struct KernelTraits
{
    ...
    // 1 warp will colaborately store (8 x 4) x 8 bf16's
    // However, this is only the CopyAtom not the TiledCopy
...
struct CollectiveEpilogue
{
    ...
    {   
        ...
        // Howeverr we wait for additional 32 threads to arrive at the barrier intentionally
        // This warp is the last warp used for the TMA store (So this warp actually arrives at the barrier twice)
...
template <
    typename Kernel_traits
>
__global__ void cute_hopper_gemm_v02(
...
{   
    ...
    void const* kernel = reinterpret_cast<void const*>(&cute_hopper_gemm_v02 <Kernel_traits>);
...
template void launch_cute_hopper_gemm_kernel_v02<cute::half_t>(size_t m, size_t n, size_t k,
